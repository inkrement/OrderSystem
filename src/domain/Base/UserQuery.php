 ...
 * @method     ChildUserQuery orderByPlz($order = Criteria::ASC) Order by the plz column
 ...
 * @method     ChildUserQuery groupByPlz() Group by the plz column
 ...
 * @method     ChildUser findOneByPlz(int $plz) Return the first ChildUser filtered by the plz column
 ...
 * @method     ChildUser requireOneByPlz(int $plz) Return the first ChildUser filtered by the plz column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUser[]|ObjectCollection findByPlz(int $plz) Return ChildUser objects filtered by the plz column
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'shop', $modelName = '\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, firstname, lastname, email, password, phone, plz, city, role FROM user WHERE id = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByPlz(1234); // WHERE plz = 1234
     * $query->filterByPlz(array(12, 34)); // WHERE plz IN (12, 34)
     * $query->filterByPlz(array('min' => 12)); // WHERE plz > 12
     * </code>
     *
     * @param     mixed $plz The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
    ...
    public function filterByPlz($plz = null, $comparison = null)
    {
        if (is_array($plz)) {
            $useMinMax = false;
            if (isset($plz['min'])) {
                $this->addUsingAlias(UserTableMap::COL_PLZ, $plz['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($plz['max'])) {
                $this->addUsingAlias(UserTableMap::COL_PLZ, $plz['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::COL_PLZ, $plz, $comparison);
    }
